...
} = require('../../shared/logic/open-meteo-location');

const app = express();
const PORT = Number(process.env.PORT || 3000);
const repoRoot = path.resolve(__dirname, '..', '..');
const publicDir = path.join(__dirname, 'public');
const sharedAssetsDir = path.join(repoRoot, 'shared', 'assets');
...
// Finds the interface the kernel would actually use to route outbound
// traffic (the default route's Iface), so multi-homed devices (Docker,
// Tailscale, Ethernet + WiFi, etc.) advertise a hub address the rest of the
// house LAN can actually reach, instead of whichever interface happened to
// enumerate first. Linux-only (/proc/net/route); returns null elsewhere or
// if parsing fails, and callers fall back to a name-based heuristic.
function getDefaultRouteInterface() {
  try {
    ...
    // Not on Linux, or /proc/net/route unavailable.
  }
  return null;
}
...
function getLocalIpv4Address() {
  ...
  if (defaultIface && interfaces[defaultIface]) {
    const onDefaultRoute = interfaces[defaultIface].find(
      (addr) => addr && addr.family === 'IPv4' && !addr.internal
    );
    if (onDefaultRoute) return onDefaultRoute.address;
  }

  // Fallback (non-Linux dev machines, or no default route found): prefer
  // typical LAN interface names over virtual/container/VPN ones, which
  // would otherwise make other clocks try to reach an address that isn't
  // actually reachable from the rest of the house LAN.
  const preferredNamePattern = /^(eth|en|wlan|wl)/i;
  const excludedNamePattern = /^(docker|br-|veth|tun|utun|tailscale|wg|zt|ppp|lo)/i;
  ...
  const preferred = candidates.find(
    (c) => preferredNamePattern.test(c.name) && !excludedNamePattern.test(c.name)
  );
  if (preferred) return preferred.address;

  const nonExcluded = candidates.find((c) => !excludedNamePattern.test(c.name));
  if (nonExcluded) return nonExcluded.address;
...
function startSharedRuntime() {
  ...
  if (!messageRuntime.monitorInterval) {
    messageRuntime.monitorInterval = setInterval(() => {
      if (messageRuntime.role === 'shared-client' && Date.now() - messageRuntime.lastHubHeartbeatAt > HEARTBEAT_STALE_MS) {
...
async function geocodeLocation(location) {
  ...
  if (!query.name) {
    throw new Error('Geocoding requires a non-empty location');
  }
  ...
  return normalizeGeocodeResult(pickBestGeocodeResult(data.results, query));
}
